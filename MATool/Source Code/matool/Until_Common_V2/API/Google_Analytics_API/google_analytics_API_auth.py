...
from Until_Common_V2 import log
...
class GAAPIAuth:
  ...
  def __init__(self,request ,site_id, redirect_uri = '', access_code = '') :
    ...
    param_ga_account_id = object_report_site_info[0]['ga_account_id']
    param_ga_view_id = object_report_site_info[0]['ga_view_id']
    param_ga_property_id = object_report_site_info[0]['ga_property_id']
    ...
    self._account_id = param_ga_account_id
    self._property_id = param_ga_property_id
    self._view_id = param_ga_view_id
  ...
  def get_authenticate_state(self, request, site_id):
    try:
      ...
      session_key_access_token = "ga_access_token_" + str(site_id)
      session_key_refresh_token = "ga_refresh_token_" + str(site_id)
      session_key_expires_in = "ga_expires_in_" + str(site_id)
      ...
      if not is_exists_session:
        ...
        if site_auth_info and site_auth_info.last_ga_refresh_token != '':
          expires_in = time_now + datetime.timedelta(minutes=50)
          refresh_token = site_auth_info.last_ga_refresh_token
          is_exists_db = True
  ...
  def get_last_access_token(self, request, site_id):
    try:
      ...
      if is_need_create_token or is_need_refresh_token :
        ...
        site_auth_info.last_ga_refresh_token = refresh_token
        #site_auth_info.last_gsc_refresh_token = ''
